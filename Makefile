...
DOCKER_ENV_STRING = -e LINODE_CLI_TOKEN -e LINODE_ROOT_PASSWORD -e WEATHER_API_TOKEN -e SSH_NGINX_LB_PUBLIC_KEY -e SSH_NGINX_LB_PRIVATE_KEY_B64 -e NGINX_LB_ROOT_PASSWORD
...
format: ## format the python code consistently
	...
	# flake8 linting
	docker container run -v $(PWD)/gwa:/gwa --entrypoint "flake8" $(DOCKER_IMAGE_NAME) --ignore=E501 /gwa/ ;\
	docker container run -v $(PWD)/gwa-deploy:/gwa-deploy --entrypoint "flake8" $(DOCKER_DEPLOY_IMAGE_NAME) --ignore=E501 /gwa-deploy
